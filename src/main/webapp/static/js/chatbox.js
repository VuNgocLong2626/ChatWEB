...
function setReceiver(element) {
	...
	var rightSide = '<div class="user-contact">' + '<div class="back">'
		...
		+ '<div class="frm-side"><input type="submit" value="Unfriend" data-receiver="'+ receiver +'" onclick="deleteFriend(this)"></div>'
		+ '</div>'
		+ '</div>'
		+ '</div>'
...
function deleteFriend(ele) {
	let receiverId = ele.getAttribute("data-receiver");

	fetch("http://" + window.location.host + "/friend-rest-controller?sender=" + username + "&receiver=" + receiverId, {
		method: 'delete'
	})
		.then(function(data) {
			rightSide.innerHTML = "";
